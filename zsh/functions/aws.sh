    ...
    case $2 in
      approve)
        aws codepipeline get-pipeline-state --name $1 --query "stageStates[*].actionStates[*].latestExecution.token" \
          --output text | xargs -I {} aws codepipeline put-approval-result --pipeline-name $1 \
...
AWScount() {
  ...
  if [[ $# -gt 0 ]]; then
    args=("${(s:/:)1}")
    if [[ $#args -gt 1 ]]; then
      echo "Setting service count of $1 to $2"
      aws ecs update-service --cluster $args[1] --service $args[2] --desired-count $2 > /dev/null
      err=$?
    else
      err=1
    fi
  ...
  if [[ $err -eq 0 ]]; then
    ...
    print -P '$0 <cluster>/<service> <count>'
  fi
}
